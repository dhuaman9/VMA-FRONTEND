...
export class RegistradorVmaGuard implements CanActivate {
  ...
  canActivate(
    ...
    return this.vmaService.isRegistroCompleto().pipe(
      map((isRegistroCreado) =>
        this.isAbleToRegister(isRegistroCreado, expectedRole)
      ),
  ...
  private isAbleToRegister = (
    isRegistroCreado: boolean,
    expectedRole: string
  ): boolean => {
    ...
    if (
      isRegistroCreado ||
      expectedRole.toUpperCase() !== userRole.toUpperCase()
    ) {
      Swal.fire({
